...
import type { AtlasWorkspaceView } from "./wbd-atlas-owner.ts";
...
export function renderOrganizationContext(topbar: string, control: ControlView, organizationId: string, capabilityCount: number, atlas?: AtlasWorkspaceView): string {
  ...
  const organizationEvidence = atlas?.evidence.filter((item) => item.organizationId === organization.id || (item.organizationId === "sportpaleis" && /sportpaleis/iu.test(organization.name))) ?? [];
  const organizationAttention = atlas?.attention.filter((item) => item.organizationId === organization.id || (item.organizationId === "sportpaleis" && /sportpaleis/iu.test(organization.name))) ?? [];
  const organizationCapabilities = [...new Set(organizationEvidence.flatMap((item) => item.capabilityRefs))];
  return `<main class="wbd-owner-workspace wbd-context-workspace">${topbar}<div class="wbd-context-worklayer"><a class="wbd-context-back" href="${organizationsPath}">← Alle organisaties</a>
    ...
    <div class="wbd-context-grid"><section class="wbd-context-panel"><header><p class="wbd-owner-eyebrow">Actuele Atlas-context</p><h2>Attention & evidence</h2></header>${organizationAttention.length ? organizationAttention.map((item) => `<article class="wbd-context-record"><span>${esc(item.type.replaceAll("_", " "))} · ${esc(item.confidence)}</span><h3>${esc(item.title)}</h3><p>${esc(item.summary)}</p></article>`).join("") : '<p class="wbd-context-empty">Geen open Attention voor deze Organization.</p>'}<details class="wbd-context-details"><summary>${organizationEvidence.length} evidence-items bekijken</summary>${organizationEvidence.slice(0, 8).map((item) => `<p><strong>${esc(item.sourceType.replaceAll("_", " "))}</strong><span>${esc(String(item.normalized.summary ?? item.normalized.title ?? item.source))} · ${esc(item.freshness)}</span></p>`).join("") || "<p>Geen gekoppelde centrale evidence.</p>"}</details></section><section class="wbd-context-panel"><header><p class="wbd-owner-eyebrow">Bewezen gebruik</p><h2>Capabilities</h2></header>${organizationCapabilities.length ? `<p>${organizationCapabilities.length} capabilities hebben evidence in deze context.</p><ul>${organizationCapabilities.map((id) => `<li>${esc(atlas?.capabilityRegistry.find((item) => item.id === id)?.name ?? id)}</li>`).join("")}</ul>` : '<p class="wbd-context-empty">Nog geen Organization–Capability-link met centrale evidence.</p>'}<a class="wbd-context-action" href="${capabilitiesPath}">Open Capability Registry</a></section></div>
    ...
    <div class="wbd-context-grid"><section class="wbd-context-panel"><header><p class="wbd-owner-eyebrow">Menselijke inspanning</p><h2>Effort</h2></header>${effortView(effort)}</section><section class="wbd-context-panel"><header><p class="wbd-owner-eyebrow">Centrale registry</p><h2>Capability-overzicht</h2></header><p>De WBD-brede registry bevat ${capabilityCount} capabilities. Alleen evidence-backed Organization-relaties worden hierboven getoond.</p><a class="wbd-context-action" href="${capabilitiesPath}">Open centrale Capability Registry</a></section></div>
    <section class="wbd-context-panel wbd-context-unknown"><header><p class="wbd-owner-eyebrow">Eerlijke grens</p><h2>Nog niet centraal beschikbaar</h2></header><p>Omgevingen, volledige historische milestones, usage en financiële bronprojecties zijn nog niet als canonieke Organization-waarheid vastgelegd.</p><p>Lokale dossiers en IndexedDB zijn niet stil geïmporteerd; ontbrekende data blijft expliciet onbekend.</p></section>
