...
function ownerMoreLinks(active: OwnerSection, atlas: AtlasWorkspaceView, includeAttention: boolean): string {
  ...
  return `${attention}<a href="${managementPath}" aria-current="${active === "management" ? "page" : "false"}"><span>Beheer</span>${decisionCount ? `<small>${decisionCount} te controleren</small>` : ""}</a><a href="${capabilitiesPath}" aria-current="${active === "capabilities" ? "page" : "false"}"><span>Capabilities</span></a><a href="${opportunitiesPath}" aria-current="${active === "opportunities" ? "page" : "false"}"><span>Kansen</span></a><a href="/workspace/experience"><span>Experiences</span><small>monitoring niet aangesloten</small></a><a href="${workContextPath}" aria-current="${active === "workcontext" ? "page" : "false"}"><span>Bestaande werkcontext</span></a>`;
}
...
function ownerTopbar(session: SessionView, active: OwnerSection, atlas: AtlasWorkspaceView): string {
  ...
  return `<header class="wbd-owner-topbar">
    <a class="wbd-owner-brand" href="${homePath}" aria-label="WBD Workspace"><span class="wbd-owner-mark" aria-hidden="true">W</span><span><strong>WBD</strong><small>Owner Workspace</small></span></a>
    <nav class="wbd-owner-primary" aria-label="Primaire WBD-navigatie"><a href="${homePath}" aria-current="${active === "home" ? "page" : "false"}"><span aria-hidden="true">◌</span>Today</a><span class="wbd-owner-sections__unavailable" aria-disabled="true" title="Mail Foundation is behouden; de echte inbox is nog niet aangesloten"><span aria-hidden="true">✉</span>Mail<small>foundation</small></span><a href="${searchPath}" aria-current="${active === "search" ? "page" : "false"}"><span aria-hidden="true">⌕</span>Search</a><span class="wbd-owner-sections__unavailable" aria-disabled="true" title="GA4 en Search Console zijn nog niet als live Owner-feed aangesloten"><span aria-hidden="true">↗</span>Growth<small>niet live</small></span><a href="${organizationsPath}" aria-current="${active === "organizations" ? "page" : "false"}"><span aria-hidden="true">◎</span>Klanten</a></nav>
    <div class="wbd-owner-actions">${decisionCount ? `<a class="wbd-owner-go-status" href="${managementPath}">${decisionCount} te controleren</a>` : ""}<details class="wbd-owner-more wbd-owner-more--desktop"><summary aria-current="${secondaryActive ? "page" : "false"}">Meer</summary><div>${ownerMoreLinks(active, atlas, true)}</div></details><div class="wbd-owner-profile"><span>${escapeHtml(session.owner.name)}</span><button type="button" data-logout>Uitloggen</button></div></div>
