...
import { materializeLegacyRollbackStateIsolated, verifyLegacyRollbackStateIsolated } from "./sportpaleis-domain-rollback-bridge.mjs";
...
// Broker-only rollback materialization runs after the production-shaped HTTP,
// preview and mutation workload and while the application would be quiesced.
// Keep its bounded child-process latency visible, but do not misclassify host
// CPU contention from that offline proof as request-event-loop latency.
const brokerOnlyEventLoopPhases = new Set(["rollback-materialization"]);
...
async function storeInitialize() {
  ...
  assert.equal(assuranceContract.limits.productionWorkerInputMaxBytes, MAX_DIRECT_PRODUCTION_WORKER_INPUT_BYTES, "runtime- en releasegrens voor workerinput wijken af");
  const alternateFont = practiceBefore.productionFonts.find(({ id, status }) => id !== font.id && status === "TECHNICALLY_VALID");
  const visualAsset = practiceBefore.productionElements.find(({ sourceId, lifecycleStatus, productionMethod }) => sourceId && lifecycleStatus === "PRODUCTION_READY" && productionMethod === "SELF_PRODUCED");
  assert.ok(alternateFont && visualAsset, "workerreachability vereist twee fonts en één productierijp visueel element");
  const workerLine = (id, source, type = "TEXT") => ({ id, type, content: id, source, widthMm: 80, heightMm: 40, quantity: 1, foilColor: "Wit", validation: { status: "VALID", reason: null } });
  const baseWorkerLine = workerLine("worker-base", { kind: "FONT", id: font.id, version: font.version, sha256: font.sha256 });
  const supplementWorkerLine = workerLine("worker-supplement", { kind: "FONT", id: alternateFont.id, version: alternateFont.version, sha256: alternateFont.sha256 });
  const visualWorkerLine = workerLine("worker-visual", { kind: "PRODUCTION_ELEMENT", id: visualAsset.id, version: visualAsset.version ?? String(visualAsset.revision), variantId: visualAsset.variants?.[0]?.id }, "LOGO");
  const workerOrder = (id, line) => ({ id, orderKind: "CUSTOM", items: [{ id: `${id}-item`, sourceType: "CUSTOM", association: "Vrije bedrukking", productionProfileId: null }], productionLines: [line] });
  const supplementProjection = projectProductionJobBuildInput({ state: practiceBefore, orders: [workerOrder("SP-WORKER-SUPPLEMENT", baseWorkerLine)], productionGroup: { lineRefs: [{ orderId: "SP-WORKER-SUPPLEMENT", lineId: baseWorkerLine.id }], supplements: [supplementWorkerLine] } });
  const visualProjection = projectProductionJobBuildInput({ state: { ...practiceBefore, productionElements: [visualAsset], productionAssetSources: [{ id: visualAsset.sourceId, original: { dataBase64: "x".repeat(2_000_000) } }] }, orders: [workerOrder("SP-WORKER-VISUAL", visualWorkerLine)], productionGroup: { lineRefs: [{ orderId: "SP-WORKER-VISUAL", lineId: visualWorkerLine.id }] } });
  const productionWorkerReachability = {
    supplementSourceIds: supplementProjection.state.productionFonts.map(({ id }) => id),
    supplementCount: supplementProjection.projection.supplementCount,
    visualSourceDocumentsTransported: visualProjection.state.productionAssetSources.length,
    supplementInputBytes: Buffer.byteLength(JSON.stringify(supplementProjection)),
    visualInputBytes: Buffer.byteLength(JSON.stringify(visualProjection)),
  };
  assert.deepEqual(new Set(productionWorkerReachability.supplementSourceIds), new Set([font.id, alternateFont.id]), "supplementbron ontbreekt in workerreachability");
  assert.equal(productionWorkerReachability.supplementCount, 1);
  ...
  const rollbackProof = await materializeLegacyRollbackStateIsolated({
    ...
    expectedDomainHash: rollbackSourceSha256,
  });
  assert.equal(rollbackProof.stateSha256, rollbackSourceSha256, "rollbackmaterialisatie rapporteerde niet de verwachte domeinhash");
  const rollbackVerification = await verifyLegacyRollbackStateIsolated({
    database: { socketPath: "/run/mysqld/mysqld.sock", name: database, user: "root" },
    organizationId: rollbackSource.organizationId,
    expectedSchemaVersion: rollbackSource.schemaVersion,
    expectedGlobalRevision: rollbackSource.revision,
    expectedDomainHash: rollbackSourceSha256,
    expectedEncodedHash: rollbackProof.encodedSha256,
  });
  const rollbackMaterializationProven = rollbackProof.stateSha256 === rollbackSourceSha256
    && rollbackVerification.independentlyDecodedSha256 === rollbackSourceSha256
    && rollbackVerification.independentlyEncodedSha256 === rollbackProof.encodedSha256
    && rollbackProof.isolatedProcessExitConfirmed === true
    && rollbackVerification.isolatedProcessExitConfirmed === true
    && Number(rollbackVerification.independentlyReadRevision) === Number(rollbackSource.revision)
    && Number(rollbackVerification.independentlyReadSchemaVersion) === Number(rollbackSource.schemaVersion);
  ...
  const productRuntimeEventLoopMaxMs = Math.max(0, ...[...phaseEventLoopMaxMs]
    .filter(([phase]) => !brokerOnlyEventLoopPhases.has(phase))
    .map(([, value]) => value));
  const metrics = { ...metricsFor(timings), eventLoopP95Ms: rounded(loop.percentile(95) / 1e6), eventLoopMaxMs: rounded(productRuntimeEventLoopMaxMs) };
  ...
  const productionWorkerInputBounded = practiceRuns.every(({ generationMetrics }) => generationMetrics?.workerTransport?.kind === "DIRECT_FREE_PRODUCTION_REACHABILITY_V1" && Number(generationMetrics.workerTransport.serializedInputBytes) <= limits.productionWorkerInputMaxBytes)
    && productionWorkerReachability.supplementCount === 1
    && productionWorkerReachability.visualSourceDocumentsTransported === 0
    && productionWorkerReachability.supplementInputBytes <= limits.productionWorkerInputMaxBytes
    && productionWorkerReachability.visualInputBytes <= limits.productionWorkerInputMaxBytes;
  ...
  const soakWorkloadCompleted = soakCycleMetrics.length === assuranceContract.minimumLoad.soakCycles
    && soakCycleMetrics.every(({ count, httpErrors, serverErrors }) => count >= assuranceContract.minimumLoad.soakRevisionPollsPerCycle + assuranceContract.minimumLoad.soakLibraryPreviewsPerCycle + assuranceContract.minimumLoad.soakBootstrapsPerCycle && httpErrors === 0 && serverErrors === 0);
  const nonEventLoopThresholdsPassed = metrics.p95Ms <= limits.allRoutesP95Ms && metrics.maxMs <= limits.allRoutesMaxMs && bootstrapMetrics.p95Ms <= limits.bootstrapP95Ms && bootstrapMetrics.maxMs <= limits.bootstrapMaxMs && scopedBootstrapPayloads && combinedRssHighWaterBytes <= limits.rssHighWaterBytes && rssRecoveredWithinBudget && practiceRuns.every(({ wallMs }) => wallMs <= 15_000) && productionBuildOffEventLoop && productionWorkerLowPriorityIsolated && productionWorkerResourcesBounded && productionWorkerOutputBounded && productionWorkerExternalRssObserved && productionWorkerRecycled && productionWorkerInputBounded && databaseConnectionReleasedDuringProductionBuild && productionPreparationDoesNotBlockMutations && rollbackMaterializationProven && soakWorkloadCompleted && soakMemoryRecovered && soakMemoryTrendStable && soakQueueStable;
  const thresholdsPassed = metrics.p95Ms <= limits.allRoutesP95Ms && metrics.maxMs <= limits.allRoutesMaxMs && bootstrapMetrics.p95Ms <= limits.bootstrapP95Ms && bootstrapMetrics.maxMs <= limits.bootstrapMaxMs && scopedBootstrapPayloads && metrics.eventLoopP95Ms <= limits.eventLoopP95Ms && metrics.eventLoopMaxMs <= limits.eventLoopMaxMs && combinedRssHighWaterBytes <= limits.rssHighWaterBytes && rssRecoveredWithinBudget && practiceRuns.every(({ wallMs }) => wallMs <= 15_000) && productionBuildOffEventLoop && productionWorkerLowPriorityIsolated && productionWorkerResourcesBounded && productionWorkerOutputBounded && productionWorkerExternalRssObserved && productionWorkerRecycled && productionWorkerInputBounded && databaseConnectionReleasedDuringProductionBuild && productionPreparationDoesNotBlockMutations && rollbackMaterializationProven && multiCycleSoakCompleted && soakMemoryRecovered && soakMemoryTrendStable && soakQueueStable;
  const result = {
    schemaVersion: 4,
