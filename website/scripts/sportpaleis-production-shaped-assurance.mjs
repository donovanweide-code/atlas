...
async function storeInitialize() {
  ...
  const rollbackRows = await pool.query("SELECT schema_version, revision, state_json FROM sp_runtime_state WHERE organization_id = ?", [rollbackSource.organizationId]);
  assert.equal(rollbackRows.length, 1, "rollbackmaterialisatie leverde niet exact één legacy-row");
  const rollbackRow = rollbackRows[0];
  const rollbackSerialized = Buffer.isBuffer(rollbackRow.state_json) ? rollbackRow.state_json.toString("utf8") : typeof rollbackRow.state_json === "string" ? rollbackRow.state_json : JSON.stringify(rollbackRow.state_json);
  const rollbackDecoded = decodeSportpaleisRuntimeState(rollbackRow.state_json);
  const rollbackDecodedSha256 = sha256CanonicalJson(rollbackDecoded);
  const rollbackEncodedSha256 = sha(rollbackSerialized);
  assert.equal(Number(rollbackRow.schema_version), Number(rollbackSource.schemaVersion), "legacy-row verloor de schemaversie");
  assert.equal(Number(rollbackRow.revision), Number(rollbackSource.revision), "legacy-row verloor de domeinrevision");
  ...
  const rollbackMaterializationProven = rollbackProof.stateSha256 === rollbackSourceSha256
    ...
    && Number(rollbackRow.revision) === Number(rollbackSource.revision);
  ...
  const result = {
    ...
    persistence: { offlineBackfill: backfillEvidence, store: storeMetricsAfterPractice, rollbackProof: { ...rollbackProof, independentlyDecodedSha256: rollbackDecodedSha256, independentlyEncodedSha256: rollbackEncodedSha256, independentlyReadRevision: Number(rollbackRow.revision) } },
