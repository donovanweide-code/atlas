  ...
  async capture(request, actor, { simulation = "success" } = {}) {
    ...
    const reserved = await this.store.mutate((state) => {
      ...
      const attempt = {
        ...
        idempotencyKey,
        payloadHash,
        contextHash: sha256(JSON.stringify(prepared.context)),
...
export function createMailOrganizations({ organizationIds } = {}) {
  ...
  const sportTemplates = {
    ...
    PROPOSAL_INTAKE_REQUEST: {
      key: "PROPOSAL_INTAKE_REQUEST", version: 1, allowedVariables: ["customer.name", "proposal.number", "proposal.title", "proposal.link", "proposal.expires"],
      subject: "Gegevens voor voorstel {{proposal.number}}",
      text: "Beste {{customer.name}},\n\nVia onderstaande veilige link kunt u de gegevens, wensen en bronbestanden voor {{proposal.title}} aanleveren.\n\n{{proposal.link}}\n\nLogo's ontvangen we bij voorkeur als EPS, AI, SVG of PDF met echte vectorinhoud. Vectorbestanden blijven scherp en zijn geschikt voor professionele bedrukking. PNG/JPG mag als referentie.\n\nDe link is geldig tot {{proposal.expires}}.\n\nMet vriendelijke groet,\nSport 2000 Sportpaleis",
      html: buildOrganizationMailShell({ brandConfig: sportpaleisBrandConfig, preheader: "Lever de gegevens en bronbestanden voor uw voorstel aan.", eyebrow: "Teamkitvoorstel {{proposal.number}}", heading: "Uw gegevens en bronbestanden", contentHtml: '<p style="margin:0 0 20px;">Beste {{customer.name}},</p><p style="margin:0 0 20px;">Via onderstaande veilige link kunt u de gegevens, wensen en bronbestanden voor <strong>{{proposal.title}}</strong> aanleveren.</p><p style="margin:0 0 20px;"><a href="{{proposal.link}}" style="display:inline-block;padding:13px 20px;background:#d71920;color:#ffffff;text-decoration:none;border-radius:4px;font-weight:bold;">Aanvraag aanvullen</a></p><p style="margin:0;">Logo\'s ontvangen we bij voorkeur als EPS, AI, SVG of PDF met echte vectorinhoud. Vectorbestanden blijven scherp en zijn geschikt voor professionele bedrukking. PNG/JPG mag als referentie.</p>', closingHtml: '<p style="margin:0 0 20px;">Deze link is geldig tot {{proposal.expires}}.</p><p style="margin:0;">Met vriendelijke groet,<br><strong>Sport 2000 Sportpaleis</strong></p>' }),
    },
    PROPOSAL_REVIEW_REQUEST: {
      key: "PROPOSAL_REVIEW_REQUEST", version: 1, allowedVariables: ["customer.name", "proposal.number", "proposal.title", "proposal.version", "proposal.link", "proposal.expires"],
      subject: "Uw teamkitvoorstel {{proposal.number}} staat klaar",
      text: "Beste {{customer.name}},\n\nVersie {{proposal.version}} van {{proposal.title}} staat voor u klaar. Bekijk de visuals, geef eventuele correcties centraal door en keur pas goed wanneer alles klopt.\n\n{{proposal.link}}\n\nDe link is geldig tot {{proposal.expires}}.\n\nMet vriendelijke groet,\nSport 2000 Sportpaleis",
      html: buildOrganizationMailShell({ brandConfig: sportpaleisBrandConfig, preheader: "Uw teamkitvoorstel staat klaar voor controle.", eyebrow: "Voorstel {{proposal.number}} · {{proposal.version}}", heading: "Bekijk uw teamkitvoorstel", contentHtml: '<p style="margin:0 0 20px;">Beste {{customer.name}},</p><p style="margin:0 0 20px;">Uw voorstel <strong>{{proposal.title}}</strong> staat klaar. Bekijk de visuals, geef eventuele correcties centraal door en keur pas goed wanneer alles klopt.</p><p style="margin:0;"><a href="{{proposal.link}}" style="display:inline-block;padding:13px 20px;background:#d71920;color:#ffffff;text-decoration:none;border-radius:4px;font-weight:bold;">Voorstel bekijken</a></p>', closingHtml: '<p style="margin:0 0 20px;">Deze link is geldig tot {{proposal.expires}}.</p><p style="margin:0;">Met vriendelijke groet,<br><strong>Sport 2000 Sportpaleis</strong></p>' }),
    },
    PROPOSAL_SUPPLIER_HANDOFF: {
      key: "PROPOSAL_SUPPLIER_HANDOFF", version: 1, allowedVariables: ["supplier.name", "proposal.number", "proposal.version", "proposal.customer", "proposal.specification"],
      subject: "Aanleverspecificatie {{proposal.number}} {{proposal.version}}",
      text: "Beste {{supplier.name}},\n\nHierbij de gecontroleerde aanleverspecificatie voor {{proposal.number}} {{proposal.version}} voor {{proposal.customer}}.\n\n{{proposal.specification}}\n\nControleer de specificatie en bevestig planning en ontvangst.\n\nMet vriendelijke groet,\nSport 2000 Sportpaleis",
      html: buildOrganizationMailShell({ brandConfig: sportpaleisBrandConfig, preheader: "Aanleverspecificatie voor externe bedrukking.", eyebrow: "Uitbesteedwerk {{proposal.number}}", heading: "Aanleverspecificatie {{proposal.version}}", contentHtml: '<p style="margin:0 0 20px;">Beste {{supplier.name}},</p><p style="margin:0 0 20px;">Hierbij de gecontroleerde aanleverspecificatie voor <strong>{{proposal.customer}}</strong>.</p><pre style="white-space:pre-wrap;margin:0;">{{proposal.specification}}</pre>', closingHtml: '<p style="margin:0 0 20px;">Controleer de specificatie en bevestig planning en ontvangst.</p><p style="margin:0;">Met vriendelijke groet,<br><strong>Sport 2000 Sportpaleis</strong></p>' }),
    },
  };
  ...
  const organizations = {
    "sportpaleis": {
      ...
      permissions: {
        preview: { SPORTPALEIS_BEDRUKKING_SMTP_TEST: ["admin"], ORDER_RECEIVED: ["admin", "store"], ORDER_IN_PRODUCTION: ["admin", "operator"], ORDER_READY: ["admin", "operator"], ORDER_QUESTION: ["admin", "operator", "store"], PROPOSAL_INTAKE_REQUEST: ["admin", "operator", "store"], PROPOSAL_REVIEW_REQUEST: ["admin", "operator"], PROPOSAL_SUPPLIER_HANDOFF: ["admin", "operator"] },
        send: { SPORTPALEIS_BEDRUKKING_SMTP_TEST: ["admin"], ORDER_RECEIVED: ["admin", "store"], ORDER_IN_PRODUCTION: ["admin", "operator"], ORDER_READY: ["admin", "operator"], ORDER_QUESTION: ["admin", "operator", "store"], PROPOSAL_INTAKE_REQUEST: ["admin", "operator", "store"], PROPOSAL_REVIEW_REQUEST: ["admin", "operator"], PROPOSAL_SUPPLIER_HANDOFF: ["admin", "operator"] },
