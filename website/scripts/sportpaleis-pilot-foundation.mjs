...
export class SportpaleisPilotService {
  ...
  #projectBootstrap({ state, user, session }, requestedSurface = "overview") {
    ...
    return {
      ...
      audit: state.audit.filter((entry) => admin || entry.userId === user.id || entry.subject.startsWith("SP-") || entry.subject === "SNIJTEST-001").slice(0, 100).map(publicBootstrapAuditEntry),
