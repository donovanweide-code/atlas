...
export async function runSportpaleisDomainBackfill(environment = process.env) {
  ...
  try {
    return await store.backfillLegacySource();
  } finally {
    await store.close();
...
async function main(environment = process.env) {
  if (environment.NODE_ENV !== "production" || environment.WBD_RELEASEBROKER_LOCK_HELD !== "true") throw new Error("Domeinbackfill vereist production mode en de releasebrokerlock.");
  const result = await runSportpaleisDomainBackfill(environment);
