...
async function request(path, body) {
  ...
  if (!result.ok) throw new Error(data.error ?? "De actie is niet gelukt.");
  return data;
}
...
function renderRows() {
  ...
  for (const row of visible) {
    if (row.orderNumber !== lastOrder) {
      ...
      html += `<section class="order-group" aria-label="Bestelling ${esc(row.orderNumber)}"><div class="order-heading"><strong>${esc(row.orderNumber)}</strong><time datetime="${esc(row.orderDate)}">${esc(dateLabel(row.orderDate))}</time></div>`;
    ...
    html += `<article class="item ${selected.has(row.id) ? "selected" : ""} ${row.excluded ? "excluded" : ""}" data-row="${row.id}">
      ...
      <div class="article"><h3>${esc(row.values.description)}</h3><div class="meta">${esc(row.values.articleNumber)}${row.club ? ` · ${esc(row.club)}` : ""}</div>${row.corrected ? '<div class="corrected">Handmatig aangepast</div>' : ""}${row.override?.structuralRuleCandidate ? '<div class="corrected">Bronfout gemarkeerd</div>' : ""}</div>
      <div class="variant"><span class="cell-label">Maat · artikelkleur</span>${esc(row.values.size)} · ${esc(row.values.color)}</div>
      ...
      <div class="item-status">${status(row)}</div><div class="item-actions">${!row.completed ? `<button data-action="edit" data-id="${row.id}">Aanpassen</button><button class="text-button" data-action="${row.excluded ? "restore" : "exclude"}" data-id="${row.id}">${row.excluded ? "Terug in batch" : "Niet meenemen"}</button>${!row.reviewed && !row.excluded ? `<button class="text-button" data-action="review" data-id="${row.id}">Gecontroleerd</button>` : ""}` : ""}</div></article>`;
  }
  if (lastOrder !== null) html += "</section>";
