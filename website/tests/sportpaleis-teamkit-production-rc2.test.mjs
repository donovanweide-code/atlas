...
test("Teamkit resolveert bestaande profielmaten en bewaart alleen expliciete overrides als approved afwijking", async (context) => {
  ...
  assert.equal(draftRules["profile-initials"].status, "RESOLVED", "de exact toegelaten Schluber-bron sluit de voormalige contourbrongap");
  ...
  assert.equal(draftRules["profile-initials"].sourceId, "font-985b2931e85cec60");
  assert.ok(draftRules["profile-initials"].sourceVersion);
  assert.ok(draftRules["profile-initials"].sourceSha256);
  ...
  assert.match(proposal.revisions.at(-1).previewHtml, /Schluber · [\d.]+×30 mm · Zwart/u, "revision-preview toont dezelfde opgeloste production truth");
  const firstPreviewItem = proposal.revisions.at(-1).previewHtml.match(/<article class="tk-item">[\s\S]*?<\/article>/u)?.[0] ?? "";
  assert.doesNotMatch(firstPreviewItem, /Productiecontrole nodig/u, "de opgeloste initialenregel toont geen oude bronwaarschuwing");
  for (const status of ["READY_FOR_REVIEW", "READY_FOR_APPROVAL"]) proposal = await service.setTeamkitProposalStatus(operator.token, operator.csrfToken, proposal.id, { status, expectedRevision: proposal.aggregateRevision });
  ...
  assert.equal(byItem["profile-back-number"].heightMm, 200, "Senior rugnummer gebruikt de actuele algemene 200-mm Product Truth");
  assert.equal(byItem["profile-back-number"].teamkitProductionContext.measurementSource, "PRODUCTION_PROFILE");
  assert.match(byItem["profile-back-number"].teamkitProductionContext.measurementEvidence, /200 mm|Product Truth|2026-09-01/iu);
