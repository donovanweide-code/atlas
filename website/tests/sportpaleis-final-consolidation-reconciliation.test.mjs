...
test("Teamwear-begeleiding, contextassets en garment surfaces blijven in de finale shell verankerd", async () => {
  ...
  assert.match(experience, /Artikelbron[\s\S]*Bedrukking[\s\S]*Maten & voorstel[\s\S]*Akkoord[\s\S]*Afhandeling/u);
  ...
  assert.match(experience, /aria-label="Artikelen in Studio"/u);
  assert.match(experience, /GARMENT_SURFACE_V1/u);
  assert.match(workspace, /Met welk artikel wil je beginnen/u);
  assert.match(workspace, /Vereniging of klant koppelen \(optioneel\)/u);
  assert.match(workspace, /Open direct in Studio/u);
