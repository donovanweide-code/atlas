...
test("Pioneers 45 rug, borst en short gebruiken nooit een generiek fallback-font", async (context) => {
  const { service, store, operator } = await fixture(context);
  ...
  assert.ok(created.productionLines.every(({ validation }) => validation.status === "BLOCKED" && /FFF englisch|productiebron/iu.test(validation.reason)));
...
test("expliciet bulk Afronden maakt alleen complete afhaalorders direct Klaar om op te halen", async (context) => {
  ...
  const created = (await service.createOrder(admin.token, admin.csrfToken, { orderKind: "INDIVIDUAL", customer: "Afronden praktijk", customerEmail: "", customerPhone: "", standardPersonalization: empty, items: [{ articleId: "sp-live-137294", size: "L", quantity: 1, deviation: false, overrides: empty }], productionLines: [{ id: "finish-initials", type: "INITIALS", content: "AF", previewLabel: "Initialen AF", widthMm: 50, heightMm: 30, quantity: 1, sourceId: font.id }] }, "practice-finish-order")).value;
