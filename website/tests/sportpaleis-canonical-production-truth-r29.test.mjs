...
test("proposal-evidence en production-ready asset blijven legitiem verschillende rollen", async (context) => {
  ...
  const knownArticle = (await service.bootstrap(operator.token)).articles.find(({ association, active }) => association === "A.S.C. Waterwijk" && active);
  assert.ok(knownArticle);
  const item = { id: "item-logo", articleId: knownArticle.id, articleNumber: knownArticle.articleNumber, productName: knownArticle.name, color: "Navy", quantity: null, sizes: [], team: null, notes: null, placements: [placement("clublogo", "CLUB_LOGO", null, { sourceId: proposalSource.source.id, productionAssetId: asset.id, assetVersion: asset.version })] };
  proposal = await approve(service, operator, proposal, [item]);
...
test("source-first creatie is atomair en retry-idempotent", async (context) => {
  ...
  const payload = { title: "Atomic source first", customerName: "Reviewklant", sources: [{ clientRef: "front", filename: "131285-front.svg", mimeType: "image/svg+xml", dataBase64: proposalSvg.toString("base64") }], items: [{ id: "item-atomic", articleId: null, articleNumber: "131285", productName: "AS'80 Academy Rugtas", color: "Zwart", quantity: null, sizes: [], team: null, notes: null, catalogSnapshot: { catalogProductId: "proposal-source:front", brand: "Eigen bron", supplierName: "Directe bron", supplierArticleName: "AS'80 Academy Rugtas", supplierArticleNumber: "131285", category: "BACKPACK", colorLabel: "Zwart", audience: [], advicePriceEur: null, effectivePriceEur: null, priceLabel: null, minimumQuantity: null, pricingPolicyRef: null, sourceAdapterId: "proposal-direct-source", sourceReference: "131285-front.svg", sourceProductId: "85835", sourceColorId: "12079", mediaClassification: "SOURCE_GALLERY_ORDER_V1", imageKey: "proposal-source:front", backImageKey: null, directFrontSourceId: null, directBackSourceId: null, directFrontSourceRef: "front", directBackSourceRef: null, productType: "BACKPACK", printableSides: ["FRONT"], sourceStatus: "AUTHORITATIVE" }, placements: [] }] };
