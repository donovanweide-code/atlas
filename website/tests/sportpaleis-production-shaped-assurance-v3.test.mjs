...
test("releaseartifact en broker binden dezelfde immutable gatecode en het drempelcontract", async () => {
  ...
  assert.match(assurance, /verifyLegacyRollbackStateIsolated/u, "de onafhankelijke rowdecode draait buiten het request-eventloop");
  assert.match(assurance, /expectedDomainHash: rollbackSourceSha256/u);
  assert.match(assurance, /expectedEncodedHash: rollbackProof\.encodedSha256/u);
  assert.match(assurance, /rollbackVerification\.independentlyDecodedSha256 === rollbackSourceSha256/u);
  ...
  assert.match(assurance, /assuranceSetupOnlyPoolPhases = new Set\(\["pool-pressure-setup"\]\)/u, "fixture-acquires zijn afzonderlijk van productpooldruk zichtbaar");
  assert.match(assurance, /assert\.equal\(pool\.taskQueueSize\?\.\(\), 0/u, "productpooldruk start uitsluitend met een lege acquirequeue");
  assert.match(assurance, /assuranceProcessQueueHighWater/u, "runner-eigen acquirewachtrij blijft als evidence zichtbaar");
  assert.match(assurance, /queueHighWaterByPhase/u, "poolwachtrij is per fase herleidbaar");
  ...
  const rollbackVerifierChild = await readFile(new URL("../scripts/sportpaleis-domain-rollback-verifier-child.mjs", import.meta.url), "utf8");
  assert.match(rollbackBridge, /isolatedProcessExitConfirmed: true/u, "parent bevestigt een terminale child-exit");
  assert.match(rollbackBridge, /sportpaleis-domain-rollback-verifier-child\.mjs/u, "rollbackrow wordt door een afzonderlijk begrensd childproces geverifieerd");
  ...
  assert.match(rollbackVerifierChild, /decodeSportpaleisRuntimeState\(row\.state_json\)/u, "verifier decodeert de werkelijk opgeslagen legacy-row");
  assert.match(rollbackVerifierChild, /await pool\.end\(\)/u, "verifier sluit de databasepool vóór zijn resultaat");
  assert.match(rollbackVerifierChild, /expectedDomainHash/u);
  assert.match(rollbackVerifierChild, /expectedEncodedHash/u);
