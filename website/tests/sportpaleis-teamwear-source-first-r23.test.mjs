...
import { generateProposalPdf, normalizeProposalItems, proposalSha256, proposalSnapshot, renderProposalPreview } from "../src/sportpaleis/teamkit-proposals.mjs";
...
test("directe productbron blijft immutable in proposal snapshot, klantpreview en PDF", async () => {
...
test("productspecifieke zijden zijn hard aan de proposal-truth gekoppeld", () => {
  ...
  assert.match(experience, /data-studio-optional-back/u, "een rugbedrukking kan de gecontroleerde achterzijde activeren zonder een beeld te verzinnen");
  assert.match(experience, /Dit product heeft geen bedrukbare achterzijde/u, "een rugtas blijft productspecifiek fail-closed");
});

test("source-first voortgang en globale planning hebben één duidelijke opslagroute", () => {
  assert.match(teamwear, /Maten & voorstel/u);
  assert.match(teamwear, /Akkoord[\s\S]*Afhandeling/u);
  assert.match(experience, /Maten en aantal opslaan/u);
});
