...
Bijsturing gereconcilieerd op 26 augustus 2026 tegen kandidaatbranch `codex/spw-final-consolidation-20260825`, live release `SPW-PREMIUM-HUMAN-EXPERIENCE-FINAL-R2-20260826`, de 97-punts consolidatie, de herstelde Teamwear-bron en de historische production-practice suites.
...
| Standalone Bedrukken | Part 1 §1–3; bijsturing §3 | Bestaande contextgestuurde ordercomposer en dezelfde productiehandoff | R2 sluit speciale create-routes uit van detailloader | first-day regression bevat expliciete routeguard | Live R2 direct `/orders/nieuw`: volledige composer, geen recoveryweergave | `/orders/nieuw` werd als order-ID `nieuw` geladen | NO |
| Vrije opdruk | Part 1 §2; bijsturing §4 | Vrije/partieel bekende opdracht gebruikt bestaande decoration- en production-foundation | R2 sluit `/orders/eigen-artikel` uit van detailloader | first-day + production suites PASS | Live R2 direct `/orders/eigen-artikel`: volledige composer, geen recoveryweergave | Speciale create-route werd als orderdetail geladen | NO |
...
| Official brand discovery | Bijsturing §15, §32–33 | Zes gecontroleerde source references in bestaande catalogusfoundation; geen feedclaim | Live R2: Stanno, Nike Teamwear, adidas Teamwear, JAKO, Robey, Craft | source-matrix regression PASS | Live Chrome R2 toont zes links, rustig-kijkenroute en directe zoekroute | Slechts één merkbron zichtbaar | NO |
...
- Candidate software gaps: geen; beide minimale R2-correcties zijn live en targeted-regression groen.
- Live core routes: desktop PASS op R2, inclusief beide directe create-routes en de zes-merkenprojectie.
