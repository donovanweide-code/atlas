...
Alleen de Owner-domainstore en bijbehorende stateprobe blijven op beide versies identiek uit de candidate afkomstig. De build bewijst via Git dat hun volledige lokale dependencygraph byte-identiek is aan de actuele LIVE-broncommit; de policy weigert deze vergelijking als een dependency is gewijzigd. De oude LIVE-bundle bevatte bijvoorbeeld de inactieve Owner-domainstore niet. De loader heeft hiervoor een expliciete lijst van infrastructuurimports; dit is geen fallback voor ontbrekende productmodules. Service, request handler en productafhankelijkheden worden wel uit de gekozen immutable runtime geladen. De vergelijking bewijst daarmee runtimegedrag bij dezelfde domain-assurance-infrastructuur, niet een loadtest van de actieve Owner legacy-store. De afzonderlijke bestaande functionaliteits-/migratiegates blijven onverkort verplicht.
